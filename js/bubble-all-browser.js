	...
	constructor(zoneSize) {
		...
		function VideoFlow(defaultVideoTag, zoneSize) {
			var calculatedCallbacks = [],
				...
				animloop = function () {
					if (isCapturing) {
						loopId = window.requestAnimationFrame(animloop);
						calculate();
					}
				};
			if (!defaultVideoTag) {
...
class Bubble {
	constructor() {
		...
		let totalMovementsThreshold=60000;
		let hideTimeout=6000;
		let repeatedActivationTime=600;
		...
		videoStream.onCalculated(function(zones) {
			...
			for (let i = 0; i < elementNames.length; i++) {
				...
				if (currentThreshold > finalThreshold){
					if ((lastActivity+repeatedActivationTime)>Date.now()) return;
